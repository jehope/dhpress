// Parallel Sets by Jason Davies, http://www.jasondavies.com/parallel-sets/
// Minor modifications by Michael Newton to support multiple values on each dimension
//    and output of selection in marker-list DIV
    ...
    function parsets(selection) {
      selection.each(function(data, i) {
        ...
        function updateDimensions() {
          ...
          tree = buildTree(tree, data, dimensions.map(dimensionName), value_, accessDim_);
          ...
          dimension = dimension.data(dimensions, dimensionName);
            ...
            // Each dimension DIV contains segments and labels
                ...
                .on("drag", function(d) {
                  ...
                  for (var i = 1; i < dimensions.length; i++) {
                    if (height * dimensions[i].y < height * dimensions[i - 1].y) {
                      ...
                      ordinal.domain([]).range(d3.range(dimensions[0].categories.length));
                      nodes = layout(tree = buildTree({children: {}, count: 0}, data, dimensionNames, value_, accessDim_), dimensions, ordinal);
                      total = getTotal(dimensions);
        ...
        function sortBy(type, f, dimension) {
          ...
          // Ribbons are the connecting bars between dimensions
    ...
    for (var i = 0; i < n; i++) {
      ...
      function buildFromNodes(startNode, dimIndex)
      {
        ...
        for (var j=0; j<categories.length; j++) {
          theCategory = categories[j];

            // If there is not yet a child node in the tree, we need to create one
            // Its children are null if we are on the last dimension (facet)
          var cNode = children.hasOwnProperty(theCategory) ? children[theCategory]
                : children[theCategory] = {
                  children: dimIndex === nd - 1 ? null : {},
                  count: 0,
                  parent: startNode,
                  dimension: dimension,
                  name: theCategory,
          ...
          buildFromNodes(cNode, dimIndex+1);
        }
      } // buildFromNodes()

        // Start from root for every data item
      buildFromNodes(root, 0);
    } // for i
