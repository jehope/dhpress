...
var map, map2,dhp_layers,dhpSettings, projectID, mObject,lookupData, lookupParents,markerObject,catFilter,player,clipPosition,dataObject,tcArray;
...
var layerCount = Object.keys(dhpData.layers).length; 
  dhp_layers = [];
    for(i=0;i<layerCount;i++) {
        ...
        if(dhpData.layers[i]['mapType']=='type-OSM') {
            dhp_layers[i] = new OpenLayers.Layer.OSM(); 
        }
        if(dhpData.layers[i]['mapType']=='type-Google') {
        ...
        if(dhpData.layers[i]['mapType']=='type-CDLA') {
